-- This is the v5 All-Devices Query with the Last Sale, card processor, pole display, food scale, and DashWeb Info columns.
...
FROM
    ...
    LEFT JOIN (
        SELECT ORG, LOCATION, KIOSK, MAX(LASTTRANSDATE) AS LASTSALE
        FROM SOSDB.KIOSKSTATUS KS
        GROUP BY ORG, LOCATION, KIOSK
...
LEFT JOIN (
	SELECT h.KIOSKNAME, REPLACE(JSON_EXTRACT(h.BASICINFO, '$.systemInfo'), CHAR(34), """") AS systemInfo,
		CASE
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon') <> ''         THEN CONCAT('bixolon : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon'), CHAR(34), ""))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp') <> ''  THEN CONCAT('seikoEpsonCorp : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp'), CHAR(34), ""))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics') <> ''   THEN CONCAT('starMicronics : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics'), CHAR(34), ""))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon') <> ''         AND JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp') <> ''		THEN CONCAT(CONCAT('seikoEpsonCorp : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp'), CHAR(34), "")), CONCAT('bixolon : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon'), CHAR(34), "")))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon') <> ''         AND JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics') <> ''		THEN CONCAT(CONCAT('starMicronics : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics'), CHAR(34), "")), CONCAT('bixolon : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon'), CHAR(34), "")))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp') <> ''	AND JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics') <> ''		THEN CONCAT(CONCAT('starMicronics : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics'), CHAR(34), "")), CONCAT('seikoEpsonCorp : ',REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp'), CHAR(34), "")))
		WHEN JSON_EXTRACT(KIOSKINFO, '$.printer.seikoEpsonCorp') <> ''	AND JSON_EXTRACT(KIOSKINFO, '$.printer.starMicronics') <> ''		AND JSON_EXTRACT(KIOSKINFO, '$.printer.bixolon') <> '' THEN REPLACE(JSON_EXTRACT(KIOSKINFO, '$.printer'), CHAR(34), "")
	END AS printer
	FROM dashdb.hostinfo h
		INNER JOIN ( SELECT KIOSKNAME, MAX(LASTUPDATED) AS LASTUPDATED
