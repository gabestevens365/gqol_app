...
SELECT
	...
	,l.LocationActive		AS	'Loc IsActive'
    ...
    ,ks.CurrentVersionValet	AS	'App Valet Version'
    ,ks.CurrentVersionOTIFirmware	AS	'App CC Firmware'
    ,''						AS	'App Anti-Malware'
    ,''						AS	'App Anti-Malware Ver'
...
INNER JOIN [365DBA].dbo.AllGmaReadiness g	ON CAST(ks.MSLID as VARCHAR(36)) = CAST(g.GroupUniqueId AS VARCHAR(36))
INNER JOIN [CUSTOMERS].dbo.CUSTOMERS c		ON ks.CustomerId = c.ID
INNER JOIN [CUSTOMERS].dbo.LOCATIONS l		ON ks.LocationId = l.ID
WHERE c.IsActive=1
